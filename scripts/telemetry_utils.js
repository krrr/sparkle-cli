...
// User-level .sparkle directory in home
const USER_SPARKLE_DIR = path.join(homedir(), SPARKLE_DIR);
// Project-level .sparkle directory in the workspace
const WORKSPACE_SPARKLE_DIR = path.join(projectRoot, SPARKLE_DIR);

// Telemetry artifacts are stored in a hashed directory under the user's ~/.sparkle/tmp
export const OTEL_DIR = path.join(USER_SPARKLE_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .sparkle directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_SPARKLE_DIR,
  'settings.json',
);
