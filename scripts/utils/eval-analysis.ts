...
import { ALL_BUILTIN_TOOL_NAMES, isValidToolName } from 'sparkle-cli-core';
...
const TOOL_NAME_TO_CONSTANT: Record<
  ...
  keyof typeof import('sparkle-cli-core')
> = {
  glob: 'GLOB_TOOL_NAME',
...
function collectImportedToolNameConstants(
  ...
  for (const statement of sourceFile.statements) {
    if (
      ...
      statement.moduleSpecifier.text !== 'sparkle-cli-core'
    ) {
      continue;
    }
