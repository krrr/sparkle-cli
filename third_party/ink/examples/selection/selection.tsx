...
const codeExample = [
	{
		...
		content: (
			<Text>
				<Text color="magenta">const</Text> <Text color="cyan">greeting</Text> ={' '}
	...
	},
	{
		...
		content: (
			<Text>
				<Text color="magenta">const</Text> <Text color="cyan">name</Text> ={' '}
	...
	},
	{
		...
		content: (
			<Text>
				<Text color="cyan">greeting</Text> + <Text color="green">", "</Text> +{' '}
				<Text color="cyan">name</Text> + <Text color="green">" (wrap)"</Text>
			</Text>
		),
	},
	{
	...
	switch (action.type) {
		...
		case 'ADD_INSERTED_LINE': {
			return {
				...
				insertedLines: [
					...state.insertedLines,
					`Inserted line ${state.insertedLines.length + 1}`,
				],
			};
		}
...
const Selection = forwardRef<SelectionReference, SelectionProperties>(
	({useStaticRender: initialUseStaticRender}, reference) => {
		...
		useEffect(() => {
			...
			if (
				state.anchorPoint &&
				!getPathToRoot(state.anchorPoint.node).includes(rootNode)
			) {
				shouldClear = true;
			}
			...
			if (
				state.focusPoint &&
				!getPathToRoot(state.focusPoint.node).includes(rootNode)
			) {
				shouldClear = true;
			}
			...
			if (state.showSelection && state.anchorPoint && state.focusPoint) {
				...
				const comparison = comparePoints(
					state.anchorPoint.node,
					state.anchorPoint.offset,
					state.focusPoint.node,
					state.focusPoint.offset,
				);
			...
			<Box flexDirection="column">
										...
										{codeExample.map((line, i) => (
											<Box key={line.id} flexDirection="row">
												<Box
													...
													flexShrink={0}
													userSelect={
														lineNumbersSelectable ? undefined : 'none'
													}
												>
				...
				<Box>
					<Text>
						...
						{state.anchorPoint
							? `${state.anchorPoint.offset}, ${
									state.anchorPoint.node.nodeName === '#text'
										? state.anchorPoint.node.nodeValue
										: state.anchorPoint.node.nodeName
								}`
							: 'undefined'}
					</Text>
				</Box>
				<Box>
					<Text>
						...
						{state.focusPoint
							? `${state.focusPoint.offset}, ${
									state.focusPoint.node.nodeName === '#text'
										? state.focusPoint.node.nodeValue
										: state.focusPoint.node.nodeName
								}`
							: 'undefined'}
					</Text>
				</Box>
				...
				<Box marginTop={1} flexDirection="column">
					...
					<Text dimColor>
						Use Arrow keys to move cursor. Shift+Arrow to select.
					</Text>
				</Box>
			</Box>
		);
