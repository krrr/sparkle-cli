	...
	if (node.yogaNode) {
		...
		if (terminalBuffer) {
			...
			const findCursor = (
				...
				const myClipW =
					Math.min(clip.x + clip.w, absX + region.width) - myClipX;
				const myClipH =
					Math.min(clip.y + clip.h, absY + region.height) - myClipY;
				...
				if (region.cursorPosition) {
					...
					if (
						cursorX >= myClipX &&
						cursorX <= myClipX + myClipW &&
						cursorY >= myClipY &&
						cursorY <= myClipY + myClipH
					) {
