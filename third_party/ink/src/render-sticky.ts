...
export function getStickyDescendants(
	node: DOMElement,
	stickyDescendants: StickyNodeInfo[] = [],
): StickyNodeInfo[] {
		...
		if (type === 'top') {
			...
			if (nextStickyNodeInfo?.resolvedInfo) {
				const nextNodeTopInViewport =
					y - currentScrollTop + nextStickyNodeInfo.resolvedInfo.stickyNodeTop;
