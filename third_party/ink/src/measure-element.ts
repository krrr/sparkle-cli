...
export const collectSortedFragments = (
	...
	const collect = (
		...
		if (
			...
			const borderBottom = currentNode.yogaNode.getComputedBorder(
				Yoga.EDGE_BOTTOM,
			);
			const borderLeft = currentNode.yogaNode.getComputedBorder(Yoga.EDGE_LEFT);
			const borderRight = currentNode.yogaNode.getComputedBorder(
				Yoga.EDGE_RIGHT,
			);
			...
			for (const child of currentNode.childNodes) {
				if (child.yogaNode) {
					const parentBorderLeft = currentNode.yogaNode.getComputedBorder(
						Yoga.EDGE_LEFT,
					);
					...
					const childX =
						...
						siblingRemovedH;
					const childY =
						y + child.yogaNode.getComputedTop() - borderTop - siblingRemovedV;
...
const handleContentMatch = (
	...
	const isVerticalMatch =
		y >= fragment.visualY && y < fragment.visualY + fragment.height;

	const isHorizontalMatch =
		x >= fragment.visualX && x <= fragment.visualX + fragment.width;
...
const getTextOffsetForTextNode = (
	node: DOMElement,
	x: number,
	y: number,
	options?: {snapToChar?: 'start' | 'end'},
...
export const hitTest = (
	...
	const {fragments} = collectSortedFragments(node);
	let bestMatch:
		| {fragment: TextFragment; vDist: number; hDist: number}
		| undefined;
