...
describe('validateNonInterActiveAuth', () => {
  ...
  function createLocalMockConfig(overrides: Partial<Config> = {}): Config {
    ...
    (config as unknown as Record<string, unknown>)['getProviderProfileService'] = vi
      .fn()
      .mockReturnValue(mockProfileService);
    return config;
  }
  ...
  it('creates transient profile and uses USE_GEMINI if GEMINI_API_KEY is set and no active profile', async () => {
    ...
    expect(mockProfileService.activateProfile).toHaveBeenCalledWith('transient-1');
  });
  ...
  it('exits if activateProfile fails', async () => {
    mockProfileService.activateProfile.mockRejectedValue(new Error('Auth error!'));
    const nonInteractiveConfig = createLocalMockConfig({
      getOutputFormat: vi.fn().mockReturnValue(OutputFormat.TEXT),
    });
    try {
  ...
  describe('JSON output mode', () => {
    it(`prints JSON error when no auth is configured and exits with code ${ExitCodes.FATAL_AUTHENTICATION_ERROR}`, async () => {
      ...
      expect(payload.error.message).toContain('Please configure a provider in your');
    });
