...
describe('initializeOutputListenersAndFlush', () => {
  ...
  it('should flush backlogs and setup listeners if no listeners exist', async () => {
    const { coreEvents } = await import('sparkle-cli-core');
...
describe('resolveSessionId', () => {
  ...
  it('should import from session file when sessionFile is provided', async () => {
    ...
    const coreModule = await import('sparkle-cli-core');
...
describe('startInteractiveUI', () => {
  ...
  it('should enable mouse events when alternate buffer is enabled', async () => {
    const { enableMouseEvents } = await import('sparkle-cli-core');
    await startTestInteractiveUI(
      mockConfig,
      mockSettings,
      mockStartupWarnings,
  ...
  it('should perform all startup tasks in correct order', async () => {
    ...
    try {
      ...
      // 5 cleanups: mouseEvents, lineWrapping, instance.unmount, TTY check,
      // and consolePatcher. The non-resumable session cleanup was removed:
      // lazy persistence never creates files for startup-only sessions.
  ...
  it('should not recordSlowRender when less than threshold', async () => {
    const { recordSlowRender } = await import('sparkle-cli-core');
