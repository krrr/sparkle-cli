...
vi.mock('sparkle-cli-core', async () => {
  const actual = await vi.importActual<typeof ServerConfig>('sparkle-cli-core');
  return {
    ...actual,
  ...
  it('should set policyUpdateConfirmationRequest if integrity MISMATCH in interactive mode when AUTO_ACCEPT is false', async () => {
    ...
    try {
      ...
      expect(config.getPolicyUpdateConfirmationRequest()).toEqual({
        scope: 'workspace',
        identifier: MOCK_CWD,
        policyDir: expect.stringContaining(path.join('.sparkle', 'policies')),
        newHash: 'new-hash',
      });
      expect(ServerConfig.createPolicyEngineConfig).toHaveBeenCalledWith(
        expect.objectContaining({
