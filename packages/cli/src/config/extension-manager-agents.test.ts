...
vi.mock('sparkle-cli-core', async (importOriginal) => {
  const core = await importOriginal<typeof import('sparkle-cli-core')>();
  return {
    ...core,
    homedir: mockHomedir,
...
describe('ExtensionManager agents loading', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sparkle-test-agents-'));
    mockHomedir.mockReturnValue(tempDir);
    vi.stubEnv('SPARKLE_CLI_HOME', tempDir);

    // Create the extensions directory that ExtensionManager expects
    extensionsDir = path.join(tempDir, '.sparkle', EXTENSIONS_DIRECTORY_NAME);
