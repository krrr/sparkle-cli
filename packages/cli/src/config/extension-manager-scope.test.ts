...
import { cleanupTmpDir } from 'sparkle-cli-test-utils';
import { loadAgentsFromDirectory, loadSkillsFromDir } from 'sparkle-cli-core';
...
describe('ExtensionManager Settings Scope', () => {
  ...
  beforeEach(async () => {
    ...
    extensionsDir = path.join(currentTempHome, '.sparkle', 'extensions');
    extensionDir = path.join(extensionsDir, extensionName);

    fs.mkdirSync(extensionDir, { recursive: true });

    // Create sparkle-extension.json
