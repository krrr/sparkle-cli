...
import { Storage } from 'sparkle-cli-core';
...
describe('ExtensionStorage', () => {
  ...
  it('should return the correct user extensions directory', () => {
    const expectedDir = path.join(mockHomeDir, '.sparkle', 'extensions');
    expect(ExtensionStorage.getUserExtensionsDir()).toBe(expectedDir);
  });

  it('should create a temporary directory', async () => {
    const mockTmpDir = '/tmp/sparkle-extension-123';
    ...
    expect(fs.promises.mkdtemp).toHaveBeenCalledWith(
      path.join('/tmp', 'sparkle-extension'),
    );
