...
import { debugLogger, getErrorMessage } from 'sparkle-cli-core';
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to Sparkle CLI',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: sparkle hooks migrate --from-claude\n\nMigrate hooks from Claude Code to Sparkle CLI format.',
      );
    }
    await exitCli();
  },
