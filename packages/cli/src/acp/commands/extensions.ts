...
import { listExtensions, type Config, getErrorMessage } from 'sparkle-cli-core';
...
export class UpdateExtensionCommand implements Command {
  ...
  async execute(
    ...
    return {
      name: this.name,
      data: 'Headless extension updating requires internal UI dispatches. Please use `sparkle extensions update` directly in the terminal.',
    };
  }
}
