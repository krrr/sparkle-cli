...
import type { HistoryTurn, Config, ResumedSessionData } from 'sparkle-cli-core';
import {
  ...
  return {
    ...
    handleDeleteSession: useCallback(
      async (session: SessionInfo) => {
        ...
        // the session, e.g. "session-<timestamp>-<sessionIdPrefix>.jsonl".
        // The ChatRecordingService.deleteSession API expects this file basename
        // (without the ".jsonl" extension), not the full session UUID.
        try {
