...
import { debugLogger, isGitRepository } from 'sparkle-cli-core';
...
export function getInstallationInfo(
  ...
  try {
    ...
    if (realPath.includes('/.volta/') || realPath.includes('/Volta/')) {
      const updateCommand = 'volta install sparkle-cli@latest';
      return {
    ...
    if (
      ...
      const updateCommand = 'pnpm add -g sparkle-cli@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add sparkle-cli@latest';
      return {
    ...
    if (realPath.includes('/.bun/install/global')) {
      const updateCommand = 'bun add -g sparkle-cli@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g sparkle-cli@latest';
    return {
