...
import { debugLogger, writeToStdout } from 'sparkle-cli-core';
...
function sanitizeNotificationContent(
  ...
  return {
    title: title || 'Sparkle CLI',
    subtitle: subtitle || undefined,
    body: body || 'Open Sparkle CLI for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'Sparkle CLI needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open Sparkle CLI to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'Sparkle CLI session complete',
