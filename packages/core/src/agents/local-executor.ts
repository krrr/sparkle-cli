...
import { SPARKLE_MODEL_ALIAS_PRO, isAutoModel } from '../config/models.js';
...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private async tryCompressChat(
    ...
    const model = this.definition.modelConfig.model ?? SPARKLE_MODEL_ALIAS_PRO;
    ...
    if (isAutoModel(requestedModel, this.context.config)) {
      ...
      if (!modelToUse) {
        try {
          ...
          modelToUse = SPARKLE_MODEL_ALIAS_PRO;
        }
