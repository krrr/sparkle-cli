...
export const SPARKLE_DIR = '.sparkle';
export const TRUSTED_FOLDERS_FILENAME = 'trustedFolders.json';

/**
 * Returns the home directory.
 * If SPARKLE_CLI_HOME environment variable is set, it returns its value.
 * Otherwise, it returns the user's home directory.
 */
export function homedir(): string {
  const envHome = process.env['SPARKLE_CLI_HOME'];
  if (envHome) {
    return envHome;
  }
  return os.homedir();
 ...
 */
export function resolveDefensiveToolPath(filePath: string, targetDir: string): string {
