 ...
 * process).
 *
 * This function can be overridden by setting the `SPARKLE_CLI_IDE_PID`
 * environment variable. This is useful for launching Sparkle CLI in a
 * standalone terminal while still connecting to an IDE instance.
 *
 * If `SPARKLE_CLI_IDE_PID` is set, the function uses that PID and fetches
 * the command for it.
 *
