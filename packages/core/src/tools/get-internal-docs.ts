...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Sparkle CLI documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Sparkle CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Sparkle CLI's internal documentation.
