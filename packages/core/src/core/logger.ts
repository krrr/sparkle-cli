...
export class Logger {
  private sparkleDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      await fs.mkdir(this.sparkleDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.sparkleDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.sparkleDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.sparkleDir) {
      debugLogger.error(
