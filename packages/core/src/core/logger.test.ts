...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_SPARKLE_DIR, { recursive: true, force: true });
  } catch {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_SPARKLE_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    it('should create .sparkle directory and an empty log file if none exist', async () => {
      const dirExists = await fs
        .access(TEST_SPARKLE_DIR)
        .then(() => true)
        .catch(() => false);
  ...
  describe('saveCheckpoint', () => {
    ...
    it.each([
      ...
      await logger.saveCheckpoint(
        { history: conversation, authType: ProviderType.USE_GEMINI },
        tag,
      );
