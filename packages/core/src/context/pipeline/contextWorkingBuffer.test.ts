...
describe('ContextWorkingBufferImpl', () => {
  it('should initialize with a pristine graph correctly', () => {
    const pristine1 = createDummyNode('ep1', NodeType.USER_PROMPT, 10, undefined, 'p1');
  ...
  it('should track 1:1 replacements (e.g., masking) and append to audit log', () => {
    ...
    const maskedNode = createDummyNode('ep1', NodeType.USER_PROMPT, 5, undefined, 'm1');
  ...
  it('should track multi-generation provenance correctly', () => {
    ...
    // Gen 2: Summarized
    const gen2 = createDummyNode('ep1', NodeType.ROLLING_SUMMARY, 5, undefined, 'gen2');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  describe('syncPristineHistory', () => {
    ...
    it('should drop working nodes if their pristine root is dropped from authoritative history', () => {
      ...
      // Mutate p2 into m2
      const m2 = createDummyNode('ep1', NodeType.AGENT_THOUGHT, 5, undefined, 'm2');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    it('should correctly weave summarized and mutated nodes into their chronological spots when new nodes arrive', () => {
      ...
      const p4 = createDummyNode('ep1', NodeType.AGENT_THOUGHT, 10, undefined, 'p4');
      const p5 = createDummyNode('ep1', NodeType.USER_PROMPT, 10, undefined, 'p5');
      ...
      expect(buffer.nodes.map((n) => n.id)).toEqual(['p1', 'm2', 'p3', 'p4', 'p5']);
    });
    it('should drop a non-pristine node if ANY of its multiple pristine roots are dropped from authoritative history', () => {
      ...
      const s1 = createDummyNode('ep1', NodeType.ROLLING_SUMMARY, 5, undefined, 's1');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
