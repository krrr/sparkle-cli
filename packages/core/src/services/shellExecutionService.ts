 ...
 * from within Sparkle CLI.
 */
export const SPARKLE_CLI_IDENTIFICATION_ENV_VAR = 'SPARKLE_CLI';

/**
 * The value of {@link SPARKLE_CLI_IDENTIFICATION_ENV_VAR}
 */
export const SPARKLE_CLI_IDENTIFICATION_ENV_VAR_VALUE = '1';
  ...
  private static async childProcessFallback(
    ...
    try {
      ...
      const handleExit = (
        ...
        if (state.truncated) {
          const truncationMessage = `\n[SPARKLE_CLI_WARNING: Output truncated. The buffer is limited to ${
    ...
    try {
      ...
      if (error?.message?.includes('posix_spawnp failed')) {
        onOutputEvent({
          type: 'data',
          chunk:
            '[SPARKLE_CLI_WARNING] PTY execution failed, falling back to child_process. This may be due to sandbox restrictions.\n',
        });
        throw e;
      } else {
        return {
