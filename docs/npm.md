...
`sparkle-cli-core`. This means that whether a user installs the package with
`npm install -g sparkle-cli` or runs it directly with `npx sparkle-cli`, they
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace sparkle-cli`.
