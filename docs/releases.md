...
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | sparkle-cli                   | @google-gemini/gemini-cli                 |
| Core       | sparkle-cli-core              | @google-gemini/gemini-cli-core A2A Server |
| A2A Server | sparkle-cli-a2a-server        | @google-gemini/gemini-cli-a2a-server      |
...
```bash
npm install -g sparkle-cli@nightly
```
...
- `npx -y sparkle-cli@latest --version` to validate the push worked as expected
  if you were not doing a rc or dev tag
- `npx -y sparkle-cli@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall sparkle-cli && npm uninstall -g sparkle-cli && npm cache clean --force &&  npm install sparkle-cli@<version>`
...
- **What happens:** The `npm publish` command is run for the `sparkle-cli-core`
  and `sparkle-cli` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g sparkle-cli` will download these packages, and `npm` will
  handle installing the `sparkle-cli-core` dependency automatically. The code in
