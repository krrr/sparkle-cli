...
Sparkle CLI can integrate with your IDE to provide a more seamless and
...
1.  **VS Code companion extension**: Install the "Sparkle CLI Companion"
...
2.  **Agent Client Protocol (ACP)**: An open protocol for interoperability
    ...
    discovery and installation of compatible agents like Sparkle CLI.

## VS Code companion extension

The **Sparkle CLI Companion extension** grants Sparkle CLI direct access to your
...
- **Native diffing:** When Sparkle suggests code modifications, you can view the
...
- **VS Code commands:** You can access Sparkle CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Sparkle CLI: Run`: Starts a new Sparkle CLI session in the integrated
    terminal.
  - `Sparkle CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `Sparkle CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Sparkle CLI: View Third-Party Notices`: Displays the third-party notices
    for the extension.
...
When you run Sparkle CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Sparkle CLI:
...
When enabled, Sparkle CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask Sparkle to modify a file, it can open a diff view directly in your
editor.
...
Sparkle CLI is officially available in the **ACP Agent Registry**. This allows
you to install and update Sparkle CLI directly within supporting IDEs and
...
Sparkle CLI is an ACP-compatible agent available in the ACP Agent Registry.
...
Any other IDE that supports the ACP Agent Registry can install Sparkle CLI
...
- **Message:**
  ...
  - **Cause:** Sparkle CLI could not find the necessary environment variables
    (`SPARKLE_CLI_IDE_WORKSPACE_PATH` or `SPARKLE_CLI_IDE_SERVER_PORT`) to
    connect to the IDE. This usually means the IDE companion extension is not
    running or did not initialize correctly.
  - **Solution:**
    1.  Make sure you have installed the **Sparkle CLI Companion** extension in
...
If automatic IDE detection fails, or if you are running Sparkle CLI in a
standalone terminal and want to manually associate it with a specific IDE
instance, you can set the `SPARKLE_CLI_IDE_PID` environment variable to the
...
When this variable is set, Sparkle CLI will skip automatic detection and attempt
...
- **Message:**
  ...
  - **Cause:** You are running Sparkle CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Sparkle CLI from the integrated terminal of a supported
