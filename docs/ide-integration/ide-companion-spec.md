...
Sparkle CLI's IDE mode. For VS Code, these features (native diffing, context
...
Sparkle CLI and the IDE plugin communicate through a local communication
channel.
...
For Sparkle CLI to connect, it needs to discover which IDE instance it's running
...
- **File naming convention:** The filename is critical and **MUST** follow the
  pattern: `sparkle-ide-server-${PID}-${PORT}.json`
...
- **Tie-breaking with environment variables (recommended):** For the most
  ...
  set the `SPARKLE_CLI_IDE_SERVER_PORT` environment variable in the integrated
  ...
  `SPARKLE_CLI_IDE_SERVER_PORT` variable to identify and connect to the correct
  window's server.
