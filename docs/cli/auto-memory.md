...
Auto Memory is an experimental feature that mines your past Sparkle CLI sessions
...
Every session you run with Sparkle CLI is recorded locally as a transcript. Auto
...
3.  Restart Sparkle CLI. The flag requires a restart because the extraction
...
6.  **Notification.** When a run produces new candidates, Sparkle CLI surfaces
    an inline message telling you how many items are waiting.
