...
You can automate tasks using a combination of Sparkle CLI and shell commands.
...
1.  Sparkle calls `run_shell_command('npm test')`.
2.  You see a confirmation prompt: `Allow command 'npm test'? [y/N]`.
3.  You press `y`.
4.  The tests run. If they fail, Sparkle reads the error output.
5.  Sparkle uses `read_file` to inspect the failing test.
6.  Sparkle uses `replace` to fix the bug.
7.  Sparkle runs `npm test` again to verify the fix.

This loop lets Sparkle work autonomously.
...
Sparkle will run the command (for example, `npm run dev`) and detach it.
