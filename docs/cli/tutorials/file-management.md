...
Explore, analyze, and modify your codebase using Sparkle CLI. In this guide,
you'll learn how to provide Sparkle CLI with files and directories, modify and
create files, and control what Sparkle CLI can see.
...
Sparkle CLI will generally try to read relevant files, sometimes prompting you
for access (depending on your settings). To ensure that Sparkle CLI uses a file,
...
If you _don't_ know the exact file path, you can ask Sparkle CLI to find it for
...
Sparkle uses the `glob` or `list_directory` tools to search your project
...
Once Sparkle CLI has context, you can direct it to make specific edits. The
...
Sparkle CLI prioritizes safety. Before any file is modified, it presents a
...
Sparkle CLI uses the `run_shell_command` tool to execute your test runner (for
example, `npm test` or `jest`). This ensures the changes didn't break existing
functionality.

## Advanced: Controlling what Sparkle sees

By default, Sparkle CLI respects your `.gitignore` file. It won't read or search
...
`.sparkleignore` file in your project root.
