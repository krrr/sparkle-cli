...
Access the live internet directly from your prompt. In this guide, you'll learn how to
search for up-to-date documentation, fetch deep context from specific URLs, and apply
...
Imagine you want to use a library released yesterday. The model doesn't know about it
yet. You need to teach it.

### Scenario: Find documentation

**Prompt:** `Search for the 'Bun 1.0' release notes and summarize the key changes.`

Sparkle uses the `web_search` tool to find relevant pages and synthesizes an answer.
This "grounding" process ensures the agent isn't hallucinating features that don't
exist.
...
Search gives you a summary, but sometimes you need the raw details. The `web_fetch` tool
lets you feed a specific URL directly into the agent's context.
...
Sparkle will retrieve the page content (stripping away ads and navigation) and use it to
...
3.  **Implement:** "Great. Now use that pattern to create an `auth.ts` file in my
    project."
...
The agent will search sources such as GitHub issues, StackOverflow, and forums to find
relevant fixes that might be too new to be in its base training set.

## Next steps

- Explore [File management](file-management.md) to see how to apply the code you
  generate.
- See the [Web search tool reference](../../tools/web-search.md) for citation details.
