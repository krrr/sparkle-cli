...
Sparkle CLI includes a Checkpointing feature that automatically saves a snapshot
...
1.  **A Git snapshot:** A commit is made in a special, shadow Git repository
    located in your home directory (`~/.sparkle/history/<project_hash>`). This
...
`~/.sparkle/tmp/<project_hash>/checkpoints`.
