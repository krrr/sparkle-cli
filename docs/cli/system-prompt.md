# System Prompt Override (SPARKLE_SYSTEM_MD)

The core system instructions that guide Sparkle CLI can be completely replaced with your
own Markdown file. This feature is controlled via the `SPARKLE_SYSTEM_MD` environment
variable.

## Overview

The `SPARKLE_SYSTEM_MD` variable instructs the CLI to use an external Markdown file for
its system prompt, completely overriding the built-in default. This is a full
replacement, not a merge. If you use a custom file, none of the original core
instructions will apply unless you include them yourself.

This feature is intended for advanced users who need to enforce strict, project-specific
behavior or create a customized persona.
...
You can set the environment variable temporarily in your shell, or persist it via a
`.sparkle/.env` file. See
...
- Use the project default path (`.sparkle/system.md`):
  ...
  - `SPARKLE_SYSTEM_MD=true` or `SPARKLE_SYSTEM_MD=1`
  - The CLI reads `./.sparkle/system.md` (relative to your current project directory).

- Use a custom file path:

  - `SPARKLE_SYSTEM_MD=/absolute/path/to/my-system.md`
...
- Disable the override (use built‑in prompt):
  - `SPARKLE_SYSTEM_MD=false` or `SPARKLE_SYSTEM_MD=0` or unset the variable.

If the override is enabled but the target file does not exist, the CLI will error with:
`missing system prompt file '<path>'`.

## Quick examples

- One‑off session using a project file:
  - `SPARKLE_SYSTEM_MD=1 sparkle`
...
- Use a custom file under your home directory:
  - `SPARKLE_SYSTEM_MD=~/prompts/system.md sparkle`

## UI indicator

When `SPARKLE_SYSTEM_MD` is active, the CLI shows a `|⌐■_■|` indicator in the UI to
signal custom system‑prompt mode.
...
- `${AgentSkills}`: Injects a complete section (including header) of all available agent
  skills.
...
Before overriding, export the current default prompt so you can review required safety
and workflow rules.

- Write the built‑in prompt to the project default path:
  - `SPARKLE_WRITE_SYSTEM_MD=1 sparkle`
- Or write to a custom path:
  - `SPARKLE_WRITE_SYSTEM_MD=~/prompts/DEFAULT_SYSTEM.md sparkle`
...
Keep system.md minimal but complete for safety and tool operation. Keep AGENTS.md
...
- Error: `missing system prompt file '…'`
  ...
  - For `SPARKLE_SYSTEM_MD=1|true`, create `./.sparkle/system.md` in your project.
- Override not taking effect
  - Confirm the variable is loaded (use `.sparkle/.env` or export in your shell).
  - Paths are resolved from the current working directory; try an absolute path.
- Restore defaults
  - Unset `SPARKLE_SYSTEM_MD` or set it to `0`/`false`.
