...
- Sparkle CLI installed and configured
...
```bash
mkdir -p .sparkle/hooks
cat > .sparkle/hooks/log-tools.sh << 'EOF'
#!/usr/bin/env bash
...
# Log to file
echo "[$(date)] Tool executed: $tool_name" >> .sparkle/tool-log.txt
...
chmod +x .sparkle/hooks/log-tools.sh
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path ".sparkle\hooks"
@"
# Read hook input from stdin
...
# Log to file
"[`$(Get-Date -Format 'o')] Tool executed: `$toolName" | Out-File -FilePath ".sparkle\tool-log.txt" -Append -Encoding utf8

# Return success with empty JSON
"{}"
"@ | Out-File -FilePath ".sparkle\hooks\log-tools.ps1" -Encoding utf8
```

## Exit Code Strategies

There are two ways to control or block an action in Sparkle CLI:
...
**`.sparkle/hooks/inject-context.sh`:**
        ...
        "hooks": [
          {
            "name": "intent-filter",
            "type": "command",
            "command": "node .sparkle/hooks/filter-tools.js"
          }
        ]
      }
    ]
...
### Configuration (`.sparkle/settings.json`)
        ...
        "hooks": [
          {
            "name": "init",
            "type": "command",
            "command": "node .sparkle/hooks/init.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "memory",
            "type": "command",
            "command": "node .sparkle/hooks/inject-memories.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "filter",
            "type": "command",
            "command": "node .sparkle/hooks/rag-filter.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "security",
            "type": "command",
            "command": "node .sparkle/hooks/security.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "record",
            "type": "command",
            "command": "node .sparkle/hooks/record.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "validate",
            "type": "command",
            "command": "node .sparkle/hooks/validate.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "save",
            "type": "command",
            "command": "node .sparkle/hooks/consolidate.js"
          }
        ]
      }
    ]
