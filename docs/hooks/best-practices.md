...
const CACHE_FILE = '.sparkle/hook-cache.json';
...
```bash
#!/usr/bin/env bash
LOG_FILE=".sparkle/hooks/debug.log"
...
# Test the hook
cat test-input.json | .sparkle/hooks/my-hook.sh
...
# Test the hook
Get-Content test-input.json | .\.sparkle\hooks\my-hook.ps1
...
input=$(cat)
echo "$input" >> .sparkle/hook-inputs.log
...
```bash
chmod +x .sparkle/hooks/*.sh
chmod +x .sparkle/hooks/*.js
...
```bash
git add .sparkle/hooks/
git add .sparkle/settings.json
...
.sparkle/hook-cache.json
.sparkle/hook-debug.log
...
| :----------------------------- | :--------------------------------------------------------------------------------------------------------------------------------- |
| **System**                     | Configured by system administrators (for example, `/etc/sparkle-cli/settings.json`, `/Library/...`). Assumed to be the **safest**. |
| **User** (`~/.sparkle/...`)    | Configured by you. You are responsible for ensuring they are safe.                                                                 |
| **Extensions**                 | You explicitly approve and install these. Security depends on the extension source (integrity).                                    |
| **Project** (`./.sparkle/...`) | **Untrusted by default.** Safest in trusted internal repos; higher risk in third-party/public repos.                               |
